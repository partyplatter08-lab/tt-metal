...
def _unpack_tilize_sweep_formats():
    return [
        f
...
def _unpack_tilize_sweep_num_faces(formats):
...
def _unpack_tilize_sweep_stoch_rnd(formats):
    if formats.output_format == DataFormat.Bfp8_b:
...
@parametrize(
    formats=_unpack_tilize_sweep_formats(),
    stoch_rnd_type=lambda formats: _unpack_tilize_sweep_stoch_rnd(formats),
    transpose=[Transpose.No],
    narrow_tile=[NarrowTile.No],
    dest_acc=[DestAccumulation.No, DestAccumulation.Yes],
    num_faces=lambda formats: _unpack_tilize_sweep_num_faces(formats),
