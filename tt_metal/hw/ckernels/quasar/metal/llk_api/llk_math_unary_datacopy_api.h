 ...
 * @tparam EN_32BIT_DEST set to true if math destination register is set to Float32/Int32 mode
 * @param operand: The input operand Data Flow Buffer identifier
 ...
 */
template <DataCopyType type, bool EN_32BIT_DEST>
inline void llk_math_eltwise_unary_datacopy_init(const std::uint32_t operand) {
    const std::uint32_t operand_id = get_operand_id(operand);
    const std::uint32_t num_faces = get_operand_num_faces(operand_id);
    const std::uint32_t face_r_dim = get_operand_face_r_dim(operand_id);
    const DataFormat src_format = static_cast<DataFormat>(get_operand_dst_format(operand_id));

    _configure_default_data_format_state_<false /* IMPLIED_MATH_FORMAT */, EN_32BIT_DEST>(src_format, src_format);
